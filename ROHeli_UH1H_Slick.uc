...
// ROHeli_UH1H_Slick
//=============================================================================
// Bell UH-1H Iroquois "Huey Slick" Transport Helicopter
//=============================================================================
// DirtyGrandpa
//=============================================================================
class ROHeli_UH1H_Slick extends ROVehicleHelicopter_Ext
	abstract;
...
// Static meshes for the door guns as seen by external viewers
...
replication
{
	...
	if (bNetDirty)
		...
		PassengerFiveCurrentPositionIndex,
...
simulated function PostBeginPlay()
{
	...
	EntryPoints[0].EntryActor.MySeatType = VST_Passenger;
}
...
simulated function VehicleWeaponFireEffects(vector HitLocation, int SeatIndex)
{
	...
	if( bUseLoopedMGSound )
	{
		if (SeatIndex == SeatIndexDoorMGLeft && SeatFiringMode(SeatIndex,,true) == 0 && !DoorMGLAmbient.IsPlaying())
		{
			if (DoorMGLStopSound.IsPlaying())
			{
				DoorMGLStopSound.StopEvents();
			}

			DoorMGLAmbient.PlayEvent(DoorMGLAmbientEvent);
		}
		else if (SeatIndex == SeatIndexDoorMGRight && SeatFiringMode(SeatIndex,,true) == 0 && !DoorMGRAmbient.IsPlaying())
		{
			if (DoorMGRStopSound.IsPlaying())
			{
				DoorMGRStopSound.StopEvents();
			}

			DoorMGRAmbient.PlayEvent(DoorMGRAmbientEvent);
		}
	}
}
...
simulated function VehicleWeaponStoppedFiring(bool bViaReplication, int SeatIndex)
{
	...
	if( bUseLoopedMGSound )
	{
		if ( SeatIndex == SeatIndexDoorMGLeft )
		{
			if ( DoorMGLAmbient.IsPlaying() )
			{
				DoorMGLAmbient.StopEvents();
				DoorMGLStopSound.PlayEvent(DoorMGLStopEvent);
			}
		}
		else if ( SeatIndex == SeatIndexDoorMGRight )
		{
			if ( DoorMGRAmbient.IsPlaying() )
			{
				DoorMGRAmbient.StopEvents();
				DoorMGRStopSound.PlayEvent(DoorMGRStopEvent);
			}
		}
	}
}
...
function DamageSeatProxy(int SeatProxyIndex, int Damage, Controller InstigatedBy, vector HitLocation, vector Momentum, class<DamageType> DamageType, optional Actor DamageCauser)
{
	...
	case 8:
		...
		ProxyHitInfo = DeathHitInfo_ProxyPassFive;
...
simulated event ReplicatedEvent(name VarName)
{
	...
	{
		PlaySeatProxyDeathHitEffects(8, DeathHitInfo_ProxyPassFive);
	}
	else
	{
...
function int GetResupplyTime()
{
	...
	if(ROHW != none && ROHW.AmmoCount < ROHW.default.MaxAmmoCount)
	{
		TimeRemaining = Max( TimeRemaining, FCeil( (ROHW.default.MaxAmmoCount - ROHW.AmmoCount) / float(M60AmmoIncr)) );
	}

	ROHW = ROHelicopterWeapon(Seats[3].Gun);
...
defaultproperties
{
	...
	Seats(1)={(	CameraTag=None,
				...
				SeatPositions=((bDriverVisible=true,bCanFlyHelo=true,bAllowFocus=false,PositionCameraTag=none,ViewFOV=0.0,
									PositionIdleAnim=copilot_Idle,DriverIdleAnim=copilot_Idle,AlternateIdleAnim=copilot_Idle_AI,SeatProxyIndex=1,
	...
	Seats(8)={(	CameraTag=None,
		CameraOffset=-420,
		SeatAnimBlendName=Pass5PositionNode,
		SeatPositions=((bDriverVisible=true,bIsExterior=true,bAllowFocus=true,PositionCameraTag=none,ViewFOV=0.0,PositionIdleAnim=Pass05_Idle,DriverIdleAnim=Pass05_Idle,AlternateIdleAnim=Pass05_Idle_AI,SeatProxyIndex=8,
							bIsExterior=true,
							PositionFlinchAnims=(Pass05_Idle),
							PositionDeathAnims=(Pass05_Death))
						),
	...
	CrewAnimSet=AnimSet'VH_VN_US_UH1H_Slick.Anim.CHR_Slick_anims'
	...
	// Muzzle Flashes
	VehicleEffects(HeliVFX_Firing1)=(EffectStartTag=UH1DoorMGL,EffectTemplate=ParticleSystem'FX_VN_Helicopters.Emitter.FX_VN_M60D_Muzzle_Flash',EffectSocket=MG_Barrel_L)
	VehicleEffects(HeliVFX_Firing2)=(EffectStartTag=UH1DoorMGR,EffectTemplate=ParticleSystem'FX_VN_Helicopters.Emitter.FX_VN_M60D_Muzzle_Flash',EffectSocket=MG_Barrel_R)
	// Shell Ejects
	VehicleEffects(HeliVFX_Firing3)=(EffectStartTag=UH1DoorMGL,EffectTemplate=ParticleSystem'FX_VN_Weapons.ShellEjects.FX_Wep_ShellEject_USA_M60_Huey',EffectSocket=ShellEjectSocket_L,bInteriorEffect=false,bNoKillOnRestart=true)
	VehicleEffects(HeliVFX_Firing4)=(EffectStartTag=UH1DoorMGR,EffectTemplate=ParticleSystem'FX_VN_Weapons.ShellEjects.FX_Wep_ShellEject_USA_M60_Huey',EffectSocket=ShellEjectSocket_R,bInteriorEffect=false,bNoKillOnRestart=true)
	// Driving effects
	...
	SeatTextureOffsets(4)=(PositionOffSet=(X=+4,Y=-18,Z=0),bTurretPosition=0)
	SeatTextureOffsets(5)=(PositionOffSet=(X=0,Y=-18,Z=0),bTurretPosition=0)
	SeatTextureOffsets(6)=(PositionOffSet=(X=-4,Y=-18,Z=0),bTurretPosition=0)
	SeatTextureOffsets(7)=(PositionOffSet=(X=-10,Y=-24,Z=0),bTurretPosition=0) //pos 8 left side
	SeatTextureOffsets(8)=(PositionOffSet=(X=+10,Y=-24,Z=0),bTurretPosition=0) //pos 9 right side
	...
	EntryPoints(1)=(CollisionRadius=13, CollisionHeight=35, AttachBone=Passenger_root, LocationOffset=(X=45,Y=0,Z=25), SeatIndex = 0) // pilot
	...
	VehHitZones(16)=(ZoneName=PASS5BODY,DamageMultiplier=1.0,VehicleHitZoneType=HVHT_CrewBody,CrewSeatIndex=8,SeatProxyIndex=8,CrewBoneName=passenger_05_HITBOX)
	VehHitZones(17)=(ZoneName=PASS5HEAD,DamageMultiplier=1.0,VehicleHitZoneType=HVHT_CrewHead,CrewSeatIndex=8,SeatProxyIndex=8,CrewBoneName=passenger_05_head_HITBOX)
	VehHitZones(18)=(ZoneName=MAINROTORBLADE2,DamageMultiplier=1.0,VehicleHitZoneType=HVHT_MainRotor,ZoneHealth=150,PhysBodyBoneName=Blade_02)
	VehHitZones(19)=(ZoneName=TAILROTORSHAFT,DamageMultiplier=1.0,VehicleHitZoneType=HVHT_TailRotorShaft,ZoneHealth=125,PhysBodyBoneName=Tail_Rotor)
	VehHitZones(20)=(ZoneName=TAILROTORBLADES,DamageMultiplier=1.0,VehicleHitZoneType=HVHT_TailRotor,ZoneHealth=125,PhysBodyBoneName=Tail_Rotor)
	VehHitZones(21)=(ZoneName=ENGINEHOUSING,DamageMultiplier=1.0,VehicleHitZoneType=HVHT_Engine,ZoneHealth=100)
	VehHitZones(22)=(ZoneName=ENGINECORE,DamageMultiplier=1.0,VehicleHitZoneType=HVHT_Engine,ZoneHealth=200,NumPensToCount=2)
	VehHitZones(23)=(ZoneName=CANOPY_FRONT_LEFT,DamageMultiplier=0.0,VehicleHitZoneType=HVHT_Canopy,ZoneHealth=20,NumPensToCount=0)
	VehHitZones(24)=(ZoneName=CANOPY_FRONT_RIGHT,DamageMultiplier=0.0,VehicleHitZoneType=HVHT_Canopy,ZoneHealth=20,NumPensToCount=0)
	VehHitZones(25)=(ZoneName=CANOPY_LEFT,DamageMultiplier=0.0,VehicleHitZoneType=HVHT_Canopy,ZoneHealth=20,NumPensToCount=0)
	VehHitZones(26)=(ZoneName=CANOPY_RIGHT,DamageMultiplier=0.0,VehicleHitZoneType=HVHT_Canopy,ZoneHealth=20,NumPensToCount=0)
	VehHitZones(27)=(ZoneName=JESUSNUT,DamageMultiplier=1.5,VehicleHitZoneType=HVHT_JesusNut,ZoneHealth=100,PhysBodyBoneName=Main_Rotor)
	VehHitZones(28)=(ZoneName=COCKPIT,DamageMultiplier=1.0,VehicleHitZoneType=HVHT_Airframe,NumPensToCount=0, PhysBodyBoneName=Fuselage)
	VehHitZones(29)=(ZoneName=FUSELAGE,DamageMultiplier=1.0,VehicleHitZoneType=HVHT_Airframe,PhysBodyBoneName=Fuselage)
	VehHitZones(30)=(ZoneName=TAILBOOM,DamageMultiplier=1.0,VehicleHitZoneType=HVHT_TailBoom,ZoneHealth=300, PhysBodyBoneName=Tail_Boom)
	VehHitZones(31)=(ZoneName=MAINROTORBLADE1,DamageMultiplier=1.0,VehicleHitZoneType=HVHT_MainRotor,ZoneHealth=150,PhysBodyBoneName=Blade_01)
	VehHitZones(32)=(ZoneName=MAINROTORSHAFT,DamageMultiplier=1.0,VehicleHitZoneType=HVHT_MainRotorShaft,ZoneHealth=150,PhysBodyBoneName=Main_Rotor)
